...
function FileIcon({ name }: { name: string }) {
  ...
  return (
    <div className={`p-2 rounded-lg flex-shrink-0 ${
      isCore ? 'bg-purple-500/10 text-purple-400' :
      isDaily ? 'bg-blue-500/10 text-blue-400' :
...
export default function Memory() {
  ...
  const { data: fileContent, loading: contentLoading } = useApi<FileContent>(
          ...
          <div className="px-5 py-3 border-b border-gray-800 flex items-center justify-between">
            ...
            {fileContent && (
              <span className="text-[10px] text-gray-500">
                Modified {formatDistanceToNow(new Date(fileContent.modified), { addSuffix: true })}
              </span>
            )}
          </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {(result.line || result.lineStart) && (
                        <span className="text-xs text-gray-500 font-mono">
                          L{result.line || result.lineStart}
                        </span>
                      )}
                      {result.score != null && (
